...
class LinkForm(Toplevel):
    def __init__(self, master, engine:Engine, cls=None, obj=None, cls2=None, *args, **kwargs):
        ...
        self.lbl_title = Label(self.f_head, text="Выберите объекты, которые хотите связать")
    ...
    def session_manager(func):
        def wrapper(self, event=None, *args, **kwargs):
            ...
            try:
                ...
                showwarning("Ошибка связывания!", "Нельзя связать куратора с группой из другой кафедры!")
                print(e, e.args)
    ...
    def on_open(self, event=None):
        ...
        for idx, obj in enumerate(objs2):
            ...
            if obj.link_types[fld] in ["many_to_one", "one_to_one"] and getattr(obj, fld) is not None or obj.depart_id != self.current_obj.id:
                continue
            text, value = (obj.id, obj.id)
...
class RemoveLinkForm(Toplevel):
    def __init__(self, master, engine:Engine, cls=None, obj=None, cls2=None, *args, **kwargs):
        ...
        self.lbl_title = Label(self.f_head, text="Выберите объекты, которые хотите отвязать")

        self.var1 = StringVar() # отвязываемый 1
        self.var2 = StringVar() # отвязываемый 2

        self.btn_mod = Button(self.f_bot, text=f"Отвязать")
        self.btn_mod.bind("<ButtonRelease>", self.on_btn_unlink_clicked,)
