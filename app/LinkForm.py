...
class LinkForm(Toplevel):
    def __init__(
        self, master, engine: Engine, cls=None, obj=None, cls2=None, *args, **kwargs
    ):
        ...
        self.lbl_title = Label(
            self.f_head, text="Выберите объекты, которые хотите связать"
        )

        self.btn_mod = Button(self.f_bot, text=f"Связать")
        self.btn_mod.bind(
            "<ButtonRelease>",
            self.on_btn_link_clicked,
        )
        ...
        self.f_col1 = [
            Frame(self.f_obj1) for _ in range(len(self.cls1.field_names.keys()))
        ]  # фрейм-колонка
        self.rbuttons1 = []
        self.tbl_elems1 = []

        self.f_col2 = [
            Frame(self.f_obj2) for _ in range(len(self.cls2.field_names.keys()))
        ]  # фрейм-колонка
        self.rbuttons2 = []
        self.tbl_elems2 = []
    ...
    def create_widgets(self):
        ...
        self.lbl_title.pack(side=TOP, expand=1, pady=5)
    ...
    def session_manager(func):
        def wrapper(self, event=None, *args, **kwargs):
            ...
            try:
                ...
                showwarning(
                    "Ошибка связывания!",
                    "Нельзя связать куратора с группой из другой кафедры!",
                )
                print(e, e.args)
    ...
    def on_btn_link_clicked(self, event):
        ...
        if obj1.depart_id != obj2.depart_id:
            raise ValueError()
    ...
    def on_open(self, event=None):
        ...
        for obj in objs1:
            if self.current_obj is not None:
                if obj.depart_id != self.current_obj.id:
                    continue
            text, value = (obj.id, obj.id)
            self.rbuttons1.append(
                Radiobutton(self.f_col1[0], text=text, value=value, variable=self.var1)
            )
            entry = [
                Label(self.f_col1[idx], text=getattr(obj, attr))
                for idx, attr in enumerate(obj.field_names.keys())
                if attr != obj.id_attr
            ]
        ...
        for idx, obj in enumerate(objs2):
            ...
            if self.current_obj is not None:
                if (
                    obj.link_types[fld] in ["many_to_one", "one_to_one"]
                    and getattr(obj, fld) is not None
                    or obj.depart_id != self.current_obj.id
                ):
                    continue
            else: 
                if (
                    ...
                    continue
            text, value = (obj.id, obj.id)
            self.rbuttons2.append(
                Radiobutton(self.f_col2[0], text=text, value=value, variable=self.var2)
            )
            entry = [
                Label(self.f_col2[idx], text=getattr(obj, attr))
                for idx, attr in enumerate(obj.field_names.keys())
                if attr != obj.id_attr
            ]
...
class RemoveLinkForm(Toplevel):
    def __init__(
        ...
        self.title(f"Удаление связей объектов {self.cls1.desc}, {self.cls2.desc}")
        ...
        self.lbl_title = Label(
            self.f_head, text="Выберите объекты, которые хотите отвязать"
        )

        self.var1 = StringVar()  # отвязываемый 1
        self.var2 = StringVar()  # отвязываемый 2

        self.btn_mod = Button(self.f_bot, text=f"Отвязать")
        self.btn_mod.bind(
            "<ButtonRelease>",
            self.on_btn_unlink_clicked,
        )
    ...
    @session_manager
    def on_btn_unlink_clicked(self, event):
        obj1 = self.session.get(self.cls1, self.var1.get())
        obj2 = self.session.get(self.cls2, self.var2.get())
        ...
        match (obj1.link_types[fld1]):
            case "one_to_many":
                getattr(obj1, fld1).remove(obj2)
                setattr(obj2, fld2, None)
            case "one_to_one":
                setattr(obj1, fld1, None)
                setattr(obj2, fld2, None)
            case "many_to_many":
                getattr(obj1, fld1).remove(obj2)
                getattr(obj2, fld2).remove(obj1)

        self.session.commit()

        showinfo("Операция выполнена успешно!", "Объекты отвязаны!")
    ...
    def on_open(self, event=None):
        ...
        for idx, obj in enumerate(objs2):
            fld = obj.linked_fields[objs1[0].__class__.__name__]
            if self.current_obj is not None:
                if getattr(obj, fld) is None or obj.depart_id != self.current_obj.id:
                    continue
            else:
                if getattr(obj, fld) is None:
                    continue
            text, value = (obj.id, obj.id)
            self.rbuttons2.append(
                Radiobutton(self.f_col2[0], text=text, value=value, variable=self.var2)
